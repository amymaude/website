...
var getEmbedCode = function(images, index){
  console.log("call to get Embed " + index)
  $.ajax({
    type: "GET",
    dataType: "jsonp",
    url:"https://api.instagram.com/oembed/?url=" + images[index].link,
    success: function(data){
      mostRecent.push(data.html);
...
$(document).ready(function(){
  $.ajax({
    ...
    success: function(data) {
      ...
      var backgroundImage1 = images[0].images.standard_resolution.url
      // $(".item img").first().attr("src", backgroundImage)
      var appendIt = function(images, func){
        images.forEach(function(image, index){getEmbedCode(images, index)});
        setTimeout(function(){func()}, 500);
      }
      appendIt(images, appendInstas);
    }
  });
...
var appendInstas = function(){
  console.log("hello")
  $('.insta-feed').append(mostRecent[0]);
  $('.item').each(function(index, value){
    $(this).append(mostRecent[index])
  })
  $('.instagram-media').css({"max-width": "500", "margin-right":"auto", "margin-left": "auto"});
